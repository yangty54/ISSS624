...
The summary report shows that the maximum neighbor distance is 72139 meter, so a maximum threshold is created, in order to make sure all units have a nearest neighbor under the maximum threshold, this maximum variable is then saved for future calculation.
...
```{r}
#| fig-cap: Figure--LISA classes with fix distance method in MAP
...
```{r}
#| fig-cap: Figure--LISA classes with adaptive distance method in MAP
